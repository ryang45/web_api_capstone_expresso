...
employeesRouter.post('/', (req, res, next) => {
  let noob = req.body.employee;
  const name = noob.name,
        position = noob.position,
        wage = noob.wage,
        isCurrentEmployee = noob.is_current_employee === 0 ? 0 : 1;
  ...
  const values = {
    ...
    $position: position,
    $wage: wage,
    $isCurrentlyEmployed: isCurrentEmployee
  };
...
employeesRouter.put('/:employeeId', (req, res, next) => {
  ...
  db.run(sql, values, (error) => {
    if (error) {
      ...
      db.get(`SELECT * FROM Employee WHERE Employee.id =
